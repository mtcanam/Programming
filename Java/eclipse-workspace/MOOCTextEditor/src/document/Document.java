...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for (int j = 0; j<wLen; j++) {
			...
			if (j>0) {
				prevChar = Character.toLowerCase(word.charAt(j-1));
			}else {
				prevChar = '.';
			}
			
			if (vowString.indexOf(c) != -1 && vowString.indexOf(prevChar) == -1) {
				//This character is a vowel, and the prev character wasn't
				sylCnt++;
				
			}
			
		}
		//Then remove a syllable if the last letter was e, and the second last was not a vowel
		if(wLen > 1 && Character.toLowerCase(word.charAt(wLen-1)) == 'e' && vowString.indexOf(Character.toLowerCase(word.charAt(wLen-2))) == -1 && sylCnt != 1) {
			sylCnt--;
		}
	...
	public double getFleschScore()
	{
		...
		double numSyl = (double)this.getNumSyllables();
		
		return (206.835-1.015*(numWords/numSent)-84.6*(numSyl/numWords));
	    //return 0.0;
	}
