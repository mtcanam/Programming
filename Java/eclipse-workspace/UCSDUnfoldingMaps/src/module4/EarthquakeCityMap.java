...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() 
	{
		...
		int quakeCount = quakeMarkers.size();
		for (Marker cm : countryMarkers) {
			...
			for (Marker qm : quakeMarkers) {
				...
				if (em.isOnLand) {
					String emCountry = (String) em.getProperty("country");
					if(emCountry.equals(countryName)) {
						cnt++;
					}
				}
			}
			if(cnt > 0) {
				quakeCount-=cnt;
				System.out.println(countryName + ": " + cnt);
			}
		}
		
		System.out.println("OCEAN QUAKES: " + quakeCount);
